    ...
    for (const grupoNome of grupos) {
      try {
        const groupResult = await client.query(
          `SELECT "Id", "Name" FROM heroku."Groups"
           WHERE LOWER(TRIM("Name")) = LOWER(TRIM($1))
           AND ("IsDeleted" IS NULL OR "IsDeleted" = false)
           LIMIT 1`,
          [grupoNome.trim()]
        ...
        if (groupResult.rows.length > 0) {
          ...
          console.log(`Group "${grupoNome}" not found in heroku."Groups"`);
        }
      } catch (groupErr) {
        console.error(`Error linking group "${grupoNome}":`, groupErr.message);
      }
    }
  }
...
app.post('/api/quests', async (req, res) => {
  ...
  try {
    ...
    const quest = await createQuest(client, player, descricao, moedas, level, ganha_carta, trilha_paciente_id, grupos);
...
app.post('/api/quests/batch', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < quests.length; i++) {
      const { nome_paciente, descricao, moedas = 2, level = 2, ganha_carta = false, trilha_paciente_id, grupos } = quests[i];
      if (!nome_paciente || !descricao) {
